...
export const useGame = () => {
  const {pontuacao, setPontuacao} = useContext(NumerosContext);
  console.log(pontuacao);
  const {movimentos, setMovimentos} = useContext(NumerosContext);
  ...
  const generateRandomTile = useCallback(() => {
    ...
    if (emptyTiles.length > 0) {
      ...
      const rndValue = ~~(Math.random()*2) ? true : false;
      var finValue = 0
      if (rndValue){
        finValue = 2;
      }
      ...
      createTile({ position, value: finValue});
    }
    ...
    for (
      ...
      availableTileIds.forEach((tileId, nonEmptyTileIndex) => {
        ...
        if (
          ...
          setPontuacao(pontuacao+currentTile.value*2);
